...
static void fors_next_leafx8(unsigned char *leaf0,
                             unsigned char *leaf1,
                             unsigned char *leaf2,
                             unsigned char *leaf3,
                             unsigned char *leaf4,
...
static void fors_gen_nodex8(unsigned char *out, const unsigned char *in, uint32_t start, uint32_t end, const unsigned char *pub_seed,
                          uint32_t fors_leaf_addrx8[8*8])
{

    memcpy(out, in, SPX_N*8);
    unsigned int j;
    ...
    {
        for(j=0;j<8;j++){
            set_tree_height(fors_leaf_addrx8+j*8, i);
        }
        fors_next_leafx8(out,out+1*SPX_N,out+2*SPX_N,out+3*SPX_N,out+4*SPX_N,out+5*SPX_N,out+6*SPX_N,out+7*SPX_N,
                            out,out+1*SPX_N,out+2*SPX_N,out+3*SPX_N,out+4*SPX_N,out+5*SPX_N,out+6*SPX_N,out+7*SPX_N,
                            ...
                            uint32_t addr_idx, void *info)
{
    ...
    for (j = 0; j < 8; j++)
    {
        ...
        set_tree_height(fors_leaf_addrx8+ j * 8, 0);
    }
    ...
    fors_gen_nodex8(leaf,leaf,1, SPX_FORS_W, pub_seed, fors_leaf_addrx8);
               ...
               const uint32_t fors_addr[8])
{
    ...
    for (i=0; i<8; i++) {
        ...
        set_type(fors_leaf_addr + 8*i, SPX_ADDR_TYPE_FORSCHAIN);
    }
